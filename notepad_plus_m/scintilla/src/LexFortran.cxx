...
static void ColouriseFortranDoc(unsigned int startPos, int length, int initStyle,
	...
	for (; sc.More(); sc.Forward()) {
		...
		if (isFixFormat && (toLineStart < 6 || toLineStart > 132)) {
	...
	for (unsigned int i = startPos; i < endPos; i++) {
		...
		if ( (style == SCE_F_WORD)  ) {
			if(iswordchar(ch) && !iswordchar(chNext)) {
                ...
                if ( strcmp( prevWord, "if")==0 ) {
                    for(k=0; k<99 ; k++) {
                        dummy[0] = static_cast<char>(tolower(styler[lastStart+k+3]));
                        if ( dummy[0] == ')' ){                                    
                            for(ii=0; ii<4 ; ii++){
                               smore[ii] = static_cast<char>(tolower(styler[lastStart+k+4+ii]));
                            }
                            smore[4] = '\0';
